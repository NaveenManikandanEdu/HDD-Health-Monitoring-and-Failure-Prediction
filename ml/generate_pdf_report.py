...
def create_pdf_report(agg, model_name, output_dir):
    """
    Generates a full PDF report using the dataframe 'agg'.
    agg columns expected: ['device_id', 'bucket', 'health_score', 'risk_score', 'dominant_mechanism']
    """
    print(f">> Generating PDF charts in: {output_dir}")
    
    # Ensure output directory exists (data/reports)
    ...
    # --- A. Donut Chart (Fixed: Clean Legend, No Overlap) ---
    ...
    plt.figure(figsize=(10, 5)) # Wider to fit legend
    
    # Create Pie Chart (Hide labels on the chart itself)
    wedges, texts, autotexts = plt.pie(
        bucket_counts, 
        labels=None,    # <--- No messy text outside
        autopct='%1.1f%%', 
        colors=chart_colors, 
        pctdistance=0.85,
        startangle=90
    ...
    # Logic: Hide percentages if they are tiny (< 2%) to avoid black blobs
    for autotext in autotexts:
        try:
            val = float(autotext.get_text().strip('%'))
            if val < 2.0:
                autotext.set_text('') # Hide
            else:
    ...
    plt.tight_layout()
    chart1_path = os.path.join(output_dir, "health_dist.png")
    ...
    if len(risky) > 0:
        ...
        plt.xlabel("Count")
        plt.tight_layout()
        chart2_path = os.path.join(output_dir, "mechanisms.png")
    ...
    plt.tight_layout()
    chart3_path = os.path.join(output_dir, "risk_dist.png")
    ...
    if os.path.exists(chart1_path):
        pdf.image(chart1_path, x=20, w=170) # Made slightly wider
        pdf.ln(5)
    ...
    # Output PDF
    pdf_name = f"HDD_Risk_Report_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf"
